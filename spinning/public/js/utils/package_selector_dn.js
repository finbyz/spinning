PackageSelector = Class.extend({
    ...
    make_dialog: function() {
        let me = this;
        let item_list = [];
        this.frm.doc.items.map(function(i) { item_list.push(i.idx + ' ' + i.item_code + ' ' + i.merge + ' ' + i.grade) });
        let fields = [{
                ...
                change: function() {
                    (me.frm.doc.items).forEach(function(d) {
                        //  me.dialog.set_values({ 'merge': '' })
                        s = me.dialog.get_value('select_item');
                        i = s.split(" ");
                        // console.log(i[0])
                        if (d.idx == i[0]) {
                    ...
                    setTimeout(function() {
                        filters['item_code'] = me.dialog.get_value('item_code');
                        filters['grade'] = me.dialog.get_value('grade');
                        filters['merge'] = me.dialog.get_value('merge');
                        filters['warehouse'] = me.dialog.get_value('warehouse');

                        me.get_packages(filters)
                    }, 400);
                    // setTimeout(function() {
                    //     //me.dialog.set_value('remove_selected', 1);
                    ...
                    //setTimeout(function () {me.set_filtered_package_data();}, 1000);
                }
            },
            ...
            },
            {
                ...
                get_query: function() {
                    ...
                    let grade_list = [...new Set((me.frm.doc.items || me.frm.doc.grade).map(function(i) {
                        return i.grade
                    }))]
...

            {
                ...
                options: 'Merge',
                default: me.merge,
                read_only: me.merge ? 1 : 1,
                reqd: 1,
                get_query: function() {
                    let item_code = me.dialog.get_value('item_code');
                    let merge_list = [...new Set((me.frm.doc.items || me.frm.doc.merge).map(function(i) { return i.merge }))]
                    return {
                        filters: {
                            "item_code": item_code,
                            'merge': ['in', merge_list]
    ...
    get_package_filters: function() {
        ...
        if (values.package) {
            filters['name'] = ["like", values.package + "%"];
        }
