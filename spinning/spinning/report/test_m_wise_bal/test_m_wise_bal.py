...
def execute(filters=None):
	...
	package_map = get_packages_map(filters, float_precision)
	...
	for item in sorted(iwb_map):
		for wh in sorted(iwb_map[item]):
			for batch in sorted(iwb_map[item][wh]):
				...
				try:
					packages = package_map[item][wh][batch].packages
					remaining_qty = package_map[item][wh][batch].remaining_qty
				except KeyError:
					packages = 0
					remaining_qty = 0
				...
				if qty_dict.opening_qty or qty_dict.in_qty or qty_dict.out_qty or qty_dict.bal_qty:
					...
					diff = flt(qty_dict.bal_qty, float_precision) - flt(remaining_qty, float_precision)
					...
					data.append([item, item_map[item]["item_name"],color,item_map[item]["item_group"],
						...
						packages, details_button, flt(remaining_qty, float_precision), diff, 
...
def get_packages_map(filters, float_precision):
	...
	for package in packages_data:
		package_map.setdefault(package.item_code, {}).setdefault(package.warehouse, {})\
			.setdefault(package.batch_no, frappe._dict({
				"remaining_qty":0.0, "packages": 0.0
			}))
		package_dict = package_map[package.item_code][package.warehouse][package.batch_no]
		package_dict.packages += 1 
		package_dict.remaining_qty += flt(package.remaining_qty, float_precision)
	return package_map

def get_package_details(filters):
	conditions = get_package_conditions(filters)

	return frappe.db.sql("""
				SELECT item_code, batch_no, warehouse, remaining_qty 
				FROM `tabPackage` as pk
				WHERE 
					status != 'Out Of Stock' %s""" % conditions, as_dict = True)
...
def get_package_conditions(filters):
	...
	if filters.get("to_date"):
		conditions += " and pk.purchase_date <= '%s'" % filters["to_date"]

	if filters.get("item_code"): conditions += " and pk.item_code = '%s' " % filters['item_code']
	if filters.get("company"): conditions += " and pk.company = '%s'" % filters["company"]
	...
	if filters.get("warehouse"):
		...
		if warehouse_details:
			conditions += " and exists (select name from `tabWarehouse` wh \
				where wh.lft >= %s and wh.rgt <= %s and pk.warehouse = wh.name)"%(warehouse_details.lft,
				warehouse_details.rgt)
