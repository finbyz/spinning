...
class MaterialRepack(Document):
	...
	def add_package_consumption(self, se):
		if self.material_unpack:
			material_unpack = frappe.get_doc("Material Unpack",self.material_unpack)
			for pkg in material_unpack.packages:
				doc = frappe.get_doc("Package", pkg.package)
				doc.add_consumption(self.doctype, self.name, pkg.net_weight)
